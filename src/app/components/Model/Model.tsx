...
export const Model = () => {
...

  {
    ...
    useWater(startContainerHeight, 'startWater', 151.0);
    useOutputButton1(
  ...
  function handle2(h: number, pipesName: Array<string>) {
    if (h === 0) {
      ...
      if (
        document.getElementById('finalWater')?.getAttribute('height') !== '240'
      )
        setEndContainerHeight(h => Math.min(h + 6 * total, maxHeight));
    }

    if (document.getElementById('finalWater')?.getAttribute('height') !== '240')
...

  {
    ...
    useWater(innerContainerAHeight, 'helpWaterA', 30.5);
    useOutputButton2(
    ...
    useWater(innerContainerBHeight, 'helpWaterB', 30.5);
    useOutputButton2(
    ...
    useWater(innerContainerCHeight, 'helpWaterC', 30.5);
    useOutputButton2(
  ...
  function handle3(h: number, pipesName: Array<string>) {
    if (h === 0) {
      ...
      if (
        document.getElementById('startWater')?.getAttribute('height') !== '240'
      )
        setStartContainerHeight(h => Math.min(h + 6, maxHeight));
    }

    if (document.getElementById('startWater')?.getAttribute('height') !== '240')
      return Math.max(h - 6, 0);
  ...
  //Use button output
  {
    ...
    useWater(endContainerHeight, 'finalWater', 286.457);
    useOutputButton3(
      endContainerHeight,
